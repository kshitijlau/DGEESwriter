...
--- The RE-ENGINEERED Master Prompt Template (Version 7.5 - Non-Negotiable Rules) ---
...
VERSION 7.5: Elevates the three core rules to a new "NON-NEGOTIABLE" section
at the top of the prompt to ensure they are strictly followed.
...
Language: The entire summary MUST be written in British English.
Structure: The entire summary MUST be a single, unified paragraph. There can be no deviation from this rule.
Competencies: You MUST NOT use the exact name of a competency (e.g., "Results Driver", "Strategic Thinker") in the narrative. Instead, you MUST describe the behavior using a verb phrase (e.g., "...demonstrated an ability to drive results," or "...showcased strategic thinking.").
Core Objective
...
---------------------------------------------
CRITICAL OPENING SENTENCE PROTOCOL (MANDATORY):
...
{salutation_name} evidenced a strong ability to [highest scoring competency verb phrase].
{salutation_name} evidenced a strong capacity to [highest scoring competency verb phrase].
{salutation_name} demonstrated a strong ability to [highest scoring competency verb phrase].
{salutation_name} demonstrated a strong capacity to [highest scoring competency verb phrase].
...
STRUCTURE AFTER OPENING: The Integrated Feedback Loop.
...
FINAL INSTRUCTIONS
---------------------------------------------
Now, process the data for {salutation_name}. Create a strict single-paragraph summary that follows all Non-Negotiable Core Rules. The first sentence MUST follow the mandatory protocol based on the single highest score. The rest of the paragraph must follow the Integrated Feedback Loop structure. The total word count should remain between 250-280 words.
"""
return prompt_text
...
messages=[
{"role": "system", "content": "You are an elite talent management consultant. Your writing is strategic and cohesive. You follow all instructions with precision, especially the format of the opening sentence and all non-negotiable core rules."},
...
st.set_page_config(page_title="DGE Executive Summary Generator v7.5", layout="wide")

st.title("📄 DGE Executive Summary Generator (V7.5)")
st.markdown("""
This application generates professional executive summaries based on leadership competency scores.
Version 7.5 includes new non-negotiable core rules for stricter adherence.

Set up your secrets.
Download the Sample Template. The format requires a salutation_name column.
...
st.download_button(
label="📥 Download Sample Template File (V7.5)",
data=sample_excel_data,
file_name="dge_summary_template_v7.5.xlsx",
...
st.dataframe(df.head())
    ...
    if st.button("Generate Summaries", key="generate"):
        ...
        if generated_summaries:
            st.balloons()
            st.subheader("Generated Summaries (V7.5)")
            ...
            st.download_button(
                label="📥 Download V7.5 Results as Excel",
                data=results_excel_data,
                file_name="Generated_Executive_Summaries_V7.5.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
